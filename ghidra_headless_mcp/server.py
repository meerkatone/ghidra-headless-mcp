...
_DESCRIPTION_OVERRIDES: dict[str, str] = {
    ...
    "program.image_base.set": "Change the program image base. The rebase is always applied to the open program in memory; commit controls whether the change is recorded in the undo stack.",
    ...
    "ghidra.call": "Invoke Ghidra or Java APIs directly through a generic bridge. Set write=true to let the call mutate programs (transitions read-only sessions to writable).",
    "ghidra.eval": "Evaluate Python code inside the live Ghidra runtime context. Set write=true to let the code mutate programs (transitions read-only sessions to writable).",
    "ghidra.info": "Return runtime information about Ghidra, PyGhidra, and the server environment.",
    "ghidra.script": "Run a Ghidra script against an open program session. Set write=true to let the script mutate the program (transitions read-only sessions to writable).",

...
_DESCRIPTION_OVERRIDES: dict[str, str] = {
    ...
    "ghidra.call": "Invoke Ghidra or Java APIs directly through a generic bridge. Raw access to a read-only session requires write=true and transitions it to writable.",
    "ghidra.eval": "Evaluate Python code inside the live Ghidra runtime context. Raw access to a read-only session requires write=true and transitions it to writable.",
    "ghidra.info": "Return runtime information about Ghidra, PyGhidra, and the server environment.",
    "ghidra.script": "Run a Ghidra script against an open program session. Raw access to a read-only session requires write=true and transitions it to writable.",
